...
router.post("/send-verification-email", [body('email').isEmail()], handleValidationErrors(), catchAsync(async function (req, res) {
    try {
        const otp = await OTP.createNew({ receiverAddress: req.body.email, purpose: otpPurpose.verification, sendingMethod: otpMethod.email });
        await otp.send();
...
// submitting otp code
router.post("/submit", [body('code').exists(), body('receiverAddress').exists()], handleValidationErrors(), catchAsync(async function (req, res) {
    try {
...
router.post("/send-recovery-email", [body('email').isEmail()], handleValidationErrors(), catchAsync(async function (req, res) {
    try {
        const otp = await OTP.createNew({ receiverAddress: req.body.email, purpose: otpPurpose.recovery, sendingMethod: otpMethod.email });
        await otp.send();
        return res.send(jsend.success({}));
    } catch (e) {
        return res.send(jsend.fail("Something went wrong", { email: e.toString() }))
